...
/*   Updated: 2025/11/17 00:58:17 by jotong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_strjoin_and_free(char **new_s, char *sub_s)
{
	char *tmp;
...
char	*extract_word(const char *s, size_t *i)
{
	...
	{
		// printf("(ew) s[%zu] = %c\n", *i, s[*i]);
		if (/*!is_in_word && */(s[*i] == '|' || s[*i] == '<' || s[*i] == '>'
			|| s[*i] == '(' || s[*i] == ')' || s[*i] == '&' || is_space(s[*i])))
			break ;
		if (s[*i] == '\\')
		{
			// printf("(ew) hit \\ \n");
			...
			{
				// (*i)++;
				ft_strjoin_and_free(&new_s, ft_strndup(s + start, *i - start));
				start = *i;
				continue;
			}
		}
